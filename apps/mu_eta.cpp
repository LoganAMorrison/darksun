...
static const std::array<std::string, 2> FNAMES = {
    std::filesystem::current_path().append("../rundata/mu_eta=0.1.csv"),
    std::filesystem::current_path().append("../rundata/mu_eta=10.0.csv")};

static boost::timer::progress_display progress(MU_ETAS.size() * NUM_N *
                                               NUM_LAM);
static std::mutex progress_mutex;

bool set_model(size_t i, DarkSunParameters &params, double mu_eta) {
  if (i < NUM_LAM * NUM_N) {
    int idx_n = i % NUM_N;
...
int main() {
  ...
  for (size_t i = 0; i < MU_ETAS.size(); i++) {
    auto f = [i](int counter, DarkSunParameters &params) {
      return set_model(counter, params, MU_ETAS[i]);
    };
