...
"""
Script for generating the plots of Td + Tsm at F.O. vs. N
"""
import os
...
if __name__ == "__main__":
    ...
    for i, file in enumerate(DATA_FILES):
        ...
        tsmfo_interp = RectBivariateSpline(ns, np.log10(lams), tsmfo_array)
        ...
        tdfo = [
            xifo_interp(x, y)[0][0] * tsmfo_interp(x, y)[0][0]
            for x, y in zip(eta_rd_cont[0], np.log10(eta_rd_cont[1]))
        ]

        ax1.plot(eta_rd_cont[0], tdfo, label=LABELS[i], ls=LINE_STYLES[i])
        ax2.plot(eta_rd_cont[0], tsmfo, label=LABELS[i], ls=LINE_STYLES[i])

    ax1.set_ylabel(r"$T_{\mathrm{d,f.o.}}$", fontsize=16)
    ax2.set_ylabel(r"$T_{\mathrm{SM,f.o.}}$", fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(
        "/home/logan/Research/DarkSun/cpp/analysis/figures/td_tsm_fo.pdf"
    )
